...
class reg_sqrt(object):
    """Save regularized sqrt, used as tf function for sqrt layer."""

    def __init__(self):

        self.sqrt_thresh_fn = 0

    def __call__(self, x_val):

        #step = tf.train.get_or_create_global_step()
        #sqrt_thresh = self.sqrt_thresh_fn(step)
        #mask = tf.cast(x_value > 0, dtype=tf.float32)
        
        output = tf.math.sqrt(tf.abs(float(x_val)))

        return output


# output_test = tf.math.sqrt(tf.abs(float(-16)))

# output_test2 = tf.math.sqrt(float(16))

# print(output_test)

# output_test.eval()
# output_test.get_shape()
