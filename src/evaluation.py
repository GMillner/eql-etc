...
class EvaluationHook(SessionRunHook):
    ...
    def before_run(self, run_context):
        if self.iteration == 0:
            ...
            tens = {}
        #return tf.train.SessionRunArgs(fetches=tens) #deprecated
        return tf.estimator.SessionRunArgs(fetches=tens)
    ...
    def end(self, session):
        if self.store_path is not None:
            ...
            np.save(path.join(self.store_path, 'kernels.npy'), kernels)
            np.save(path.join(self.store_path, 'biases.npy'), biases)
            #mlflow.log_metric("kernels", kernels)
            #mlflow.log_metric("biases", biases)
            
            # # add list of weights during training
            # if os.path.isfile(path.join(self.store_path, 'list_kernels.npy')):
            #     list_kernels = list(np.load(path.join(self.store_path, 'list_kernels.npy')), allow_pickle=True)
            #     list_biases = list(np.load(path.join(self.store_path, 'list_biases.npy')), allow_pickle=True)
            # else:
            #     list_kernels = []
            #     list_biases = []
            
            # list_kernels.append(kernels)
            # list_biases.append(biases)
            # np.save(path.join(self.store_path, 'list_kernels.npy'), list_kernels)
            # np.save(path.join(self.store_path, 'list_biases.npy'), list_biases)
...
class PretrainHook(SessionRunHook):
    """Hook for pre training the eql."""
    ...
    def end(self, session):
        if self.store_path is not None:
            ...
            kernels = [value for key, value in self.weights.items() if 'kernel' in key.lower()]
            biases = [value for key, value in self.weights.items() if 'bias' in key.lower()]
            np.save(path.join(self.store_path, 'kernels_pretrain.npy'), kernels)
            np.save(path.join(self.store_path, 'biases_pretrain.npy'), biases)
            self.complexity = calculate_complexity(kernels, biases, self.fns_list, self.thresh)
...
def set_pretrain_hook(num_h_layers, model_dir, **_):
    kernel_tensornames = [weight_name_for_i(i, 'kernel') for i in range(num_h_layers + 1)]
    bias_tensornames = [weight_name_for_i(i, 'bias') for i in range(num_h_layers + 1)]
    if not os.path.exists(model_dir+'\\pre_train'):
        os.makedirs(model_dir+'\\pre_train')
    symbolic_hook = EvaluationHook([*kernel_tensornames, *bias_tensornames], store_path=model_dir+'\\pre_train')
    return symbolic_hook
...
def num_of_weights(kernels, biases):
    """count number of weights (non-zero elements in weight matrixes).
    ...
    Returns:
        number_of_nonzero: number of non-zero elements in kernels & biases, int
    ...
    for i in range(len(kernels)):
        number_of_nonzero = number_of_nonzero + (kernels[i] != 0).sum(0).sum(0) + (biases[i] != 0).sum(0).sum(0)
        #print(number_of_nonzero)
    
    return number_of_nonzero  

def create_symbolic_expression(kernels, biases, fns_list, round_decimals):
    """
    Creates a symbolic expression of network
    :param kernels: list of 2D numpy arrays
    :param biases: list of 1D numpy arrays
    ...
    for i, result in enumerate(res):
        result = round_sympy_expr(result, round_decimals) # changed (GMi)
        #np.save(path.join(save_path, 'formula_not_simplified_' + str(i) + '.npy'), result) 
        ...
        #np.save(path.join(save_path, 'formula_' + str(i) + '.npy'), result)
        return result
...
def save_symbolic_expression(kernels, biases, fns_list, save_path, round_decimals):
    ...
    :param round_decimals: integer specifying to which decimal the expression is rounded
    """
    num_of_arg = num_of_weights(kernels, biases)
    print('Number of arguments in formula expression before simplification: ', num_of_arg)
    
    if num_of_arg >= 500:
        print('save_symbolic_expression determined due to too large formula expression (num_of_arg > 500)')
        np.save(path.join(save_path, 'formula_0.npy'), np.nan)
        np.save(path.join(save_path, 'error_dumbfile_formula_above_500_args.npy'), num_of_arg)
    else:
        in_nodes = get_symbol_list(kernels[0].shape[0])
        res = in_nodes
        for kernel, bias, fns in zip(kernels, biases, fns_list):
        ...
        #round_sympy_expr(result, round_decimals) # original
        result = round_sympy_expr(res[0], round_decimals) # changed (GMi) - round decimals works now
        np.save(path.join(save_path, 'formula_not_simplified_0.npy'), result)
        np.save(path.join(save_path, 'formula_without_rounding_0.npy'), res)
        ...
        try:
            ...
            print('Simplification of result y failed. Saving representations of non-simplified formula.')
            
        end = time.time()
        
        print('...after simplification - Time (s): '+str(end - start))
        
        np.save(path.join(save_path, 'formula_0.npy'), result)    
        #expr_to_latex_png(res, path.join(save_path, 'latex_y.png'))
        
        if num_of_arg > 250:
            np.save(path.join(save_path, 'error_dumbfile_formula_above_250_args.npy'), num_of_arg)
        
        if num_of_arg <= 250: #causes process error when result is too big
            #expression_graph_as_png(result, path.join(save_path, 'graph_y.png'), view=False)
            print('save_symbolic_expression succesfull')
