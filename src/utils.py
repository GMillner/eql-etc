...
from src.config import network_parameters
...
# with argparse
#def update_runtime_params(argv, argv_names, params):
...
#            #params.update(literal_eval(argv[i]))
#            param_dict[argv_names[i]] =  argv[i]
#            print('!!!!!!!!!!!!!!!!!!!!!!!!!!!' + argv_names[i])
#            print('!!!!!!!!!!!!!!!!!!!!!!!!!!!' + argv[i])
...
def update_runtime_params(argv, params):
    ...
    params.update(network_parameters)
    if len(argv) > 1:
        params.update(literal_eval(argv[2]))
    params['model_dir'] = path.join(params['model_base_dir'], str(params['id']))
    return params

def update_runtime_params_nomlflow(argv, params):
    """Routine to update the default parameters with network_parameters and parameters from commandline."""
    params.update(network_parameters)
    if len(argv) > 1:
